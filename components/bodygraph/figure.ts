...
/**
 * Where the figure sits in the BodyGraph's 620 x 840 space: bounding box
 * placed at 4,4 and running to 616,836.
 *
 * THE SCALE IS NOT UNIFORM, and that is a considered choice rather than an
 * oversight. The artwork's own aspect is 0.820 wide-to-tall; the space it has
 * to fill is 0.735. Scaling uniformly leaves exactly two options and both are
 * worse:
 *
 *   - fit the WIDTH and the figure is 747 tall, too short to reach from the
 *     Head centre down past the Root, which would leave the bottom of the Root
 *     hanging off the lap;
 *   - fit the HEIGHT and the figure is 682 wide, so about 31 units of hand is
 *     clipped off each side by the viewBox.
 *
 * The remaining alternative — widening the BodyGraph's viewBox to 682 and
 * moving every gate — shrinks every centre and numeral by 9% for the same
 * container width, which costs legibility on a phone.
 *
 * So the figure is stretched about 10% vertically instead. On a flat
 * decorative silhouette that reads as a slightly taller person and is not
 * visible without an A/B; on the drawing's own terms it is the smallest of
 * the three compromises.
 */
export const FIGURE_TRANSFORM = {
  scaleX: 612 / FIGURE_ARTWORK_BOX.width,
  scaleY: 832 / FIGURE_ARTWORK_BOX.height,
  get translateX(): number {
    return 4 - FIGURE_ARTWORK_BOX.minX * this.scaleX;
  },
  get translateY(): number {
    return 4 - FIGURE_ARTWORK_BOX.minY * this.scaleY;
  },
} as const;

/** The transform as an SVG attribute value. */
export function figureTransformAttr(): string {
  const { translateX, translateY, scaleX, scaleY } = FIGURE_TRANSFORM;
  return `translate(${translateX.toFixed(4)} ${translateY.toFixed(4)}) scale(${scaleX.toFixed(6)} ${scaleY.toFixed(6)})`;
}

/** Map a point from the artwork's coordinates into the BodyGraph's. */
export function placeFigurePoint(x: number, y: number): { x: number; y: number } {
  const { translateX, translateY, scaleX, scaleY } = FIGURE_TRANSFORM;
  return { x: translateX + x * scaleX, y: translateY + y * scaleY };
}
